...
const BottomNavBar = () => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-gray-900/95 backdrop-blur-sm border-t border-teal-500/20 lg:hidden z-50">
      <div className="flex items-center justify-center py-2 px-2 sm:px-4">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`relative flex flex-col items-center py-2 px-1 sm:px-2 md:px-3 rounded-lg transition-all duration-300 ease-in-out transform ${
                  item.isActive
                    ? 'text-teal-400 scale-110'
                    : 'text-gray-400 hover:text-teal-300 hover:scale-105'
                } ${item.isHome ? 'mx-1 sm:mx-2 md:mx-4' : ''}`}
              >
                ...
                <div className={`p-1.5 sm:p-2 rounded-xl transition-all duration-300 ${
                  ...
                  <IconComponent className={`transition-all duration-300 ${
                    item.isHome ? 'h-5 w-5 sm:h-6 sm:w-6' : 'h-4 w-4 sm:h-5 sm:w-5'
                  }`} />
                </div>
                <span className={`text-xs font-medium mt-1 transition-all duration-300 text-center leading-tight ${
                  item.isHome ? 'font-semibold' : ''
                }`}>
                  {item.label}
                </span>
              </Link>
